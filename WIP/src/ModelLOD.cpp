/*
  Copyright (C) 2015 Jonathan Flynn
...
//----------------------------------------------------------------------------------------------------------------------
/// @file ModelLOD.cpp
/// @brief implementation files for ModelLOD class
//----------------------------------------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------------------------------------
// parse face
void ModelLOD::parseFace(const char * _begin   )
{
...
//----------------------------------------------------------------------------------------------------------------------
bool ModelLOD::load(const std::string &_fname,bool _calcBB )
{
 // here we build up our ebnf rules for parsing
  ...
  srule vertex= ("v"  >> spt::real_p >> spt::real_p >> spt::real_p) [bind(&ModelLOD::parseVertex,boost::ref(*this), _1)];
  /// our tex rule and binding of the parse function
  srule tex= ("vt" >> spt::real_p >> spt::real_p) [bind(&ModelLOD::parseTextureCoordinate, boost::ref(*this), _1)];
  // the normal rule and parsing function
  srule norm= ("vn" >> spt::real_p >> spt::real_p >> spt::real_p) [bind(&ModelLOD::parseNormal,boost::ref(*this), _1)];

  // our vertex data can be any of the above values
  srule vertex_type = vertex | tex | norm;

  // the rule for the face and parser
  srule  face = (spt::ch_p('f') >> *(spt::anychar_p))[bind(&ModelLOD::parseFace, boost::ref(*this), _1)];
...
//----------------------------------------------------------------------------------------------------------------------
ModelLOD::ModelLOD( const std::string& _fname  ) :AbstractMesh()
{
    m_vbo=false;
    m_ext=0;
    // set default values
...
//----------------------------------------------------------------------------------------------------------------------
ModelLOD::ModelLOD( const std::string& _fname,const std::string& _texName   ):AbstractMesh()
{
    m_vbo=false;
    m_vao=false;
    m_ext=0;
...
//----------------------------------------------------------------------------------------------------------------------
void ModelLOD::save(const std::string& _fname)const
{
